...
def main():
    dir = os.path.join("finalized_logs", "bas-on-gain-6")
    ...
    for j, vel in enumerate(velocities): 
        ...
        for i in np.linspace(1, number_of_files, number_of_files, dtype=int):
            ...
            eigenvalues_real.append(popt[3])
        ...
        avg_eigenvalues_img.append(sum(eigenvalues_real)/np.size(eigenvalues_real))
    ...
    plt.plot(
        velocities_ms, avg_eigenvalues_real, '.', label="Measured real eigenvalues"
    )
    plt.plot(
        velocities_ms, avg_eigenvalues_img, '.', label='Measured imaginary eigenvalues'
    )
    plt.legend()
